...
export function AuthFlow({ initialView = "landing", onLogin, onStaffLogin, onSignup }: Props) {
  const [view, setView] = useState<"landing" | "auth">(initialView);
  ...
  return (
    <main className={`auth-shell auth-reference auth-shell-motion ${mode === "signup" ? "auth-shell-signup" : ""} ${loginStep === "staff" ? "auth-shell-staff" : ""}`}>
      ...
      <section className={`auth-card ${mode === "signup" ? "auth-card-wide" : ""}`}>
        ...
        <header className="auth-reference-brand"><span className="auth-form-mark" aria-hidden="true"><HydraMark /></span><strong>Hydra Agro</strong><small>GESTÃO RURAL INTELIGENTE</small></header>
        {mode === "login" ? <div className="auth-content auth-enter" key={loginStep}>
          {loginStep === "email" ? <form onSubmit={submitLogin} className="auth-login-form">
            <h1>Bem-vindo de volta</h1><p className="auth-subtitle">Acesse sua conta para continuar.</p>
            <Field label="E-mail"><input type="email" value={email} onChange={(event) => { setEmail(event.target.value); setError(""); }} placeholder="seu@email.com" autoComplete="email" required /></Field>
            <Field label="Senha"><div className="input-with-action"><input type={showPassword ? "text" : "password"} value={password} onChange={(event) => { setPassword(event.target.value); setError(""); }} placeholder="Sua senha" autoComplete="current-password" required /><button type="button" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"}>{showPassword ? <EyeOff size={19} /> : <Eye size={19} />}</button></div></Field>
            <button className="text-button auth-forgot" type="button" onClick={() => { setLoginStep("recovery"); setError(""); }}>Esqueci minha senha</button>
            {error && <p className="form-error" role="alert">{error}</p>}
            <button className="primary-button full" type="submit" disabled={submitting}>{submitting ? "Entrando…" : "Entrar"}<ArrowRight size={20} /></button>
            <div className="auth-divider"><span>ou</span></div>
            
            <button className="text-button" type="button" onClick={() => void sendLoginCode()} disabled={submitting}>Entrar com código de e-mail</button>
            <p className="auth-switch">Ainda não tem uma conta? <button type="button" onClick={() => switchMode("signup")}>Criar conta</button></p>
            <button className="text-button" type="button" onClick={() => { setLoginStep("staff"); setError(""); }}>Acesso de funcionário</button>
          </form>
          ...
          <div className="signup-topline"><button className="auth-back" type="button" onClick={() => { setView("landing"); switchMode("login"); }}><ArrowLeft size={17} /> Voltar</button><div className="step-dots" aria-label={`Etapa ${signupStep + 1} de 5`}>{[0,1,2,3,4].map((step) => <span key={step} aria-current={step === signupStep ? "step" : undefined} className={`${step === signupStep ? "active" : ""} ${step < signupStep ? "done" : ""}`}>{step + 1}</span>)}</div></div>
