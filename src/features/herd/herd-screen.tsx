...
import { BadgeCheck, Camera, ChevronDown, ChevronRight, Beef as Cow, Crown, Filter, History, LoaderCircle, LockKeyhole, Nfc, Pencil, Plus, Search, Sprout, Trash2, Weight } from "lucide-react";
...
export function HerdScreen({ account, updateAccount, openNfc, focusAnimalId, saveAnimalPhoto, createRequest, onRequestHandled }: Props) {
  ...
  async function saveAnimal(event: FormEvent) {
    ...
    if (!animal.identification.trim()) { setError("Informe a identificação do animal."); return; }
    ...
    if (duplicate) { setError("Já existe um animal com esta identificação."); return; }
    ...
    try {
      await updateAccount((current) => {
        ...
        return { ...current, animals: current.animals.map((existing) => {
      ...
      setAnimal(blankAnimal); setEditingId(undefined); setFormOpen(false);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Não foi possível salvar o animal."); }
  ...
  async function saveWeight(event: FormEvent) {
    ...
    if (!Number.isFinite(parsed) || parsed <= 0) { setWeightError("Informe um peso válido maior que zero."); return; }
    if (!weightDraft.date) { setWeightError("Informe a data da pesagem."); return; }
    setWeightSaving(true); setWeightError("");
    try {
      const note = weightDraft.note.trim();
      const entry: AnimalHistoryEntry = { id: makeId("history"), date: `${weightDraft.date}T12:00:00`, type: "Pesagem", description: note ? `Peso registrado: ${parsed} kg · ${note}` : `Peso registrado: ${parsed} kg`, weight: parsed };
      await updateAccount((current) => ({ ...current, animals: current.animals.map((item) => item.id === selected.id ? { ...item, weight: parsed, history: [...(item.history ?? []), entry] } : item) }), { requireRemote: true });
      setWeightOpen(false); setWeightDraft({ date: today(), weight: "", note: "" }); showAppToast("Pesagem registrada no histórico");
    } catch (caught) { setWeightError(caught instanceof Error ? caught.message : "Não foi possível registrar a pesagem."); }
  ...
  async function removeAnimal(item: Animal) {
    ...
    try {
      ...
      setSelectedId(undefined); setDeleteTarget(null); showAppToast("Animal excluído com sucesso");
    } catch (caught) { setDeleteError(caught instanceof Error ? caught.message : "Não foi possível excluir o animal."); }
  ...
  return (
    <div className="screen page-enter">
      ...
      <button className="nfc-inline-card" onClick={() => openNfc()}><span><Nfc size={23} /></span><div><strong>Localizar por NFC/RFID</strong><small>Aproxime uma tag ou digite o código.</small></div><ChevronRight size={19} /></button>
      ...
      <button className="nfc-inline-card herd-tools-toggle" onClick={() => setToolsOpen((value) => !value)} aria-expanded={toolsOpen}><span>{isPlus ? <Sprout size={23} /> : <Crown size={23} />}</span><div><strong>Ferramentas do rebanho</strong><small>{isPlus ? "Nutrição, produção, reprodução e saúde" : "Recursos avançados do Hydra Agro+"}</small></div><ChevronDown size={19} style={{ transform: toolsOpen ? "rotate(180deg)" : undefined }} /></button>

      {toolsOpen && (isPlus ? <div className="herd-tools-expanded">
        <button className="herd-care-launch" onClick={() => setCareOpen(true)}><span><Sprout size={23} /></span><div><strong>Nutrição, alimentação e manejo</strong><small>Orientações, habitat e bem-estar por espécie.</small></div><ChevronRight size={19} /></button>
      ...
      </div> : <section className="herd-plus-lock" aria-label="Recursos Hydra Agro+">
        <header><span><Crown size={20} /></span><div><small>HYDRA AGRO+</small><strong>Ferramentas avançadas do rebanho</strong></div></header>
        <div className="herd-plus-lock-grid"><div><Sprout size={18} /><span><strong>Nutrição e manejo</strong><small>Orientações e acompanhamento</small></span><LockKeyhole size={15} /></div><div><Cow size={18} /><span><strong>Produção do rebanho</strong><small>Leite, ovos, mel e registros</small></span><LockKeyhole size={15} /></div><div><History size={18} /><span><strong>Reprodução</strong><small>Prenhez e inseminação</small></span><LockKeyhole size={15} /></div><div><BadgeCheck size={18} /><span><strong>Saúde e vacinas</strong><small>Calendário e acompanhamento</small></span><LockKeyhole size={15} /></div></div>
        <p>Disponível no Hydra Agro+. A gestão básica do animal e o NFC continuam gratuitos.</p>
      </section>)}

      <Modal open={careOpen && isPlus} onClose={() => setCareOpen(false)} eyebrow="HYDRA AGRO+" title="Nutrição, alimentação e manejo" wide tall><HerdCareGuide account={account} /></Modal>
      ...
      <Modal open={formOpen} onClose={() => { setFormOpen(false); setError(""); }} eyebrow={editingId ? "EDIÇÃO" : "NOVA FICHA"} title={editingId ? "Editar animal" : "Cadastrar animal"} wide dismissible={!saving}>
        <GuidedForm className="modal-form" onSubmit={saveAnimal}>
      ...
      <Modal open={Boolean(selected)} onClose={() => { setSelectedId(undefined); setError(""); setWeightOpen(false); }} eyebrow="FICHA INDIVIDUAL" title={selected?.name || selected?.identification || "Animal"} dismissible={!photoBusy && !weightSaving}>
        {selected && <div className="animal-detail">
          ...
          {selected.weight && <div className="detail-line"><Weight size={19} /><div><span>Último peso informado</span><strong>{selected.weight} kg</strong></div></div>}{selected.notes && <div className="detail-note">{selected.notes}</div>}
          {isPlus ? <><AnimalPublicShare animal={selected} /><WeightEvolution animal={selected} onAdd={() => openWeight(selected)} />{(selected.history?.length ?? 0) > 0 && <div className="animal-history animal-timeline"><h3><History size={17} /> Linha do tempo</h3>{selected.history!.slice().reverse().map((entry) => <div key={entry.id} className={entry.weight ? "is-weight" : ""}><span /><p><strong>{entry.type}</strong>{entry.description}{entry.weight && <b>{entry.weight} kg</b>}<small>{new Date(entry.date).toLocaleString("pt-BR")}</small></p></div>)}</div>}</> : <div className="animal-plus-lock"><Crown size={21} /><div><strong>Histórico avançado no Hydra Agro+</strong><small>Evolução de peso, linha do tempo completa e compartilhamento público/QR.</small></div><LockKeyhole size={16} /></div>}
          {error && <p className="form-error" role="alert">{error}</p>}<div className="detail-actions three"><button className="secondary-button" onClick={() => openEdit(selected)}><Pencil size={17} /> Editar</button><button className="secondary-button" onClick={() => openNfc(selected.id)}><Nfc size={17} /> Vincular tag</button><button className="danger-button" onClick={() => { setDeleteError(""); setDeleteTarget(selected); }}><Trash2 size={17} /> Excluir</button></div>
        </div>}
      </Modal>
      ...
      <Modal open={weightOpen && Boolean(selected) && isPlus} onClose={() => { setWeightOpen(false); setWeightError(""); }} eyebrow="HYDRA AGRO+ · PESAGEM" title="Registrar novo peso" dismissible={!weightSaving}>
        <GuidedForm className="modal-form" onSubmit={saveWeight}><div className="weight-modal-animal"><Weight size={22} /><span><strong>{selected?.name || selected?.identification}</strong><small>O registro entra automaticamente na linha do tempo.</small></span></div><div className="field-combo"><Field label="Data"><input type="date" value={weightDraft.date} onChange={(event) => setWeightDraft({ ...weightDraft, date: event.target.value })} /></Field><Field label="Peso (kg)"><input inputMode="decimal" value={weightDraft.weight} onChange={(event) => { setWeightDraft({ ...weightDraft, weight: event.target.value }); setWeightError(""); }} placeholder="Ex.: 245" autoFocus /></Field></div><Field label="Observação (opcional)"><textarea value={weightDraft.note} onChange={(event) => setWeightDraft({ ...weightDraft, note: event.target.value })} placeholder="Ex.: pesagem após manejo" /></Field>{weightError && <p className="form-error" role="alert">{weightError}</p>}<div className="modal-action-row"><button className="secondary-button" type="button" onClick={() => setWeightOpen(false)} disabled={weightSaving}>Cancelar</button><LoadingButton className="primary-button" type="submit" loading={weightSaving} loadingLabel="Registrando pesagem...">Registrar peso</LoadingButton></div></GuidedForm>
      </Modal>
...
function WeightEvolution({ animal, onAdd }: { animal: Animal; onAdd: () => void }) {
  ...
  const max = Math.max(...weights, 1); const min = Math.min(...weights, max); const range = Math.max(max - min, 1); const first = weights[0] ?? 0; const last = weights[weights.length - 1] ?? 0; const delta = points.length > 1 ? last - first : 0;
  return <section className="weight-evolution-card"><header><div><span>EVOLUÇÃO DO PESO</span><strong>{last ? `${last} kg` : "Sem pesagens"}</strong>{points.length > 1 && <small className={delta >= 0 ? "positive" : "negative"}>{delta >= 0 ? "+" : ""}{delta.toFixed(1).replace(".0", "")} kg no período</small>}</div><button onClick={onAdd}><Plus size={16} /> Pesagem</button></header>{points.length > 0 ? <div className="weight-chart" role="img" aria-label="Gráfico da evolução do peso do animal">{points.map((entry) => { const height = points.length === 1 ? 68 : 30 + (((entry.weight ?? 0) - min) / range) * 62; return <div className="weight-chart-point" key={entry.id}><span className="weight-chart-value">{entry.weight} kg</span><i style={{ height: `${height}%` }} /><small>{new Date(entry.date).toLocaleDateString("pt-BR", { day: "2-digit", month: "short" })}</small></div>; })}</div> : <div className="weight-chart-empty"><Weight size={22} /><span><strong>Comece o histórico de pesagem</strong><small>Registre o peso para acompanhar a evolução deste animal.</small></span></div>}</section>;
}
