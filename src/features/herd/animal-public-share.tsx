    ...
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} eyebrow="HYDRA ID" title="Identidade digital do animal" wide dismissible={!writing && !photoBusy}>
        <div className="public-share-modal">
          ...
          <div className="public-share-photo-status ready"><MapPin size={18} /><span><strong>{originBusy ? "Buscando propriedade…" : origin.propertyName || "Propriedade de origem"}</strong><small>{origin.municipality ? `${origin.municipality} · será exibido na Hydra ID` : "A origem é buscada automaticamente no cadastro da propriedade."}</small></span></div>

          {hasPhoto && <div className={`public-share-photo-status ${publicPhotoPath ? "ready" : photoError ? "error" : ""}`}>{photoBusy ? <LoaderCircle size={18} className="spin" /> : <ImageIcon size={18} />}<span><strong>{publicPhotoPath ? "Foto sincronizada" : photoError ? "Foto não incluída" : "Atualizando foto"}</strong><small>{publicPhotoPath ? "A Hydra ID usa a foto mais recente do animal." : photoError ? "O link continuará funcionando sem a imagem." : "Criando a cópia pública mais recente desta imagem."}</small></span></div>}
