...
function isDictationTarget(element: Element | null): element is DictationTarget {
  ...
  return ["text", "search", "tel", "number"].includes(element.type || "text");
}

function firstVisibleDictationTarget(scope: ParentNode = document) {
  const candidates = Array.from(scope.querySelectorAll("input, textarea, select"));
  return candidates.find((candidate) => isDictationTarget(candidate) && (candidate as HTMLElement).offsetParent !== null) as DictationTarget | undefined;
}

function findDictationTarget(button: HTMLButtonElement) {
  const container = button.closest(".easy-field");
  if (container) {
    const local = Array.from(container.querySelectorAll("input, textarea, select")).find(isDictationTarget);
    if (local) return local;
  }
  if (isDictationTarget(document.activeElement)) return document.activeElement;
  const modal = document.querySelector(".modal-layer .modal-body");
  return firstVisibleDictationTarget(modal || document) || null;
}
...
export function EasyModeProvider({ children, accountId }: { children: ReactNode; accountId: string }) {
  ...
  return <EasyContext.Provider value={{ enabled, setEnabled }}>{children}{enabled && <EasyVoiceDock />}</EasyContext.Provider>;
}
...
export function VoiceDictationButton({ label, showText = false }: { label: string; showText?: boolean }) {
  ...
  return <div className="easy-dictation-wrap"><button type="button" className={`easy-dictation secondary-button ${listening ? "is-listening" : ""}`} aria-label={listening ? `Ouvindo ${label}` : `Falar para preencher ${label}`} onClick={(event) => {
    if (listening) {
    ...
    if (!target) {
      const message = "Toque primeiro no campo que você quer preencher.";
      setError(message);
      speak(message);
      return;
    }
    ...
    if (!Recognition) {
      const message = "Ditado por voz não disponível neste aparelho.";
      setError(message);
      speak(message);
      return;
    }
  ...
  }}><Mic size={22} />{showText && <span>{listening ? "Ouvindo…" : "Falar"}</span>}</button>{error && <small className="easy-assist-error" role="status">{error}</small>}</div>;
}

function EasyVoiceDock() {
  const [availableField, setAvailableField] = useState(false);
  useEffect(() => {
    const update = () => setAvailableField(Boolean(isDictationTarget(document.activeElement) || firstVisibleDictationTarget(document.querySelector(".modal-layer .modal-body") || document)));
    update();
    document.addEventListener("focusin", update);
    document.addEventListener("focusout", update);
    const observer = new MutationObserver(update);
    observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["hidden", "disabled", "open"] });
    return () => {
  ...
  return <aside className="easy-voice-dock" aria-label="Ajuda para preencher por voz"><VoiceDictationButton label="o campo atual" showText /><span className="easy-voice-dock-hint">Fale em vez de escrever</span></aside>;
}
...
export function EasyHome({ navigate }: { navigate: (route: AppRoute) => void }) {
  const intro = "Modo Fácil. Escolha uma função. O botão com alto-falante lê as informações. Quando aparecer um microfone perto de um campo, toque nele e fale para preencher sem precisar escrever.";
  return <section className="screen easy-home"><header className="easy-home-header"><div className="easy-home-title-row"><div><span>MODO FÁCIL</span><h1>O que você quer fazer?</h1><p>Ícones grandes, ajuda por áudio e ditado por voz.</p></div><AudioHelp text={intro} /></div><div className="easy-voice-tip"><Mic size={19} /><div><strong>Você pode falar em vez de escrever</strong><small>Quando o microfone aparecer, toque e fale normalmente.</small></div></div></header><div className="easy-shortcuts">{shortcuts.map(({ route, title, text, icon: Icon }) => <div key={route}><button className="secondary-button" onClick={() => navigate(route)}><span className="easy-shortcut-icon"><Icon size={29} /></span><strong>{title}</strong><ChevronRight className="easy-shortcut-arrow" size={22} /></button><AudioHelp text={text} /></div>)}</div></section>;
}
