...
export function CommunityScreen({ account, onBack, publishPost, likePost, commentPost, deletePost, refreshCommunity, createRequest, onRequestHandled }: Props) {
  ...
  async function confirmPublish() {
    ...
    if (result.ok) {
      ...
      setMessage("");
      showAppToast("Publicado na comunidade");
      return;
    }
  ...
  return (
    <div className="screen page-enter extra-screen community-screen">
      <ScreenHeader
        title="Comunidade"
        subtitle="Converse com quem também vive o campo."
        onBack={onBack}
        action={(
          <div className="header-action-pair">
            <button className="icon-button" onClick={() => void refresh()} aria-label="Atualizar comunidade" disabled={busy === "refresh"}>
              {busy === "refresh" ? <LoaderCircle size={19} className="spin" /> : <RefreshCw size={19} />}
            </button>
      ...
      <div className="community-tabs">
        ...
        <button className={filter === "region" ? "active" : ""} onClick={() => setFilter("region")}>Perto de mim</button>
      ...
      <button className="community-composer" onClick={() => setComposerOpen(true)}>
        ...
        <p>Tem algo para compartilhar hoje?</p>
        <ImagePlus size={21} />
      </button>
      ...
      {filteredPosts.length === 0 ? (
        <EmptyState
          ...
          title={account.posts.length ? "Nada por aqui neste filtro" : "A comunidade ainda está quieta"}
          text={filter === "region" && !account.property.municipality ? "Complete a localização da propriedade para ver publicações da sua região." : "Quando quiser, compartilhe uma dúvida, descoberta ou momento da propriedade."}
          action={<button className="primary-button" onClick={() => setComposerOpen(true)}>Fazer a primeira publicação</button>}
        />
      ) : (
            ...
            <article className="post-card" key={post.id}>
              <header>
                ...
                <div>
                  ...
                  <small>{[post.propertyName, [post.municipality, post.state].filter(Boolean).join(", ")].filter(Boolean).join(" · ") || new Date(post.date).toLocaleString("pt-BR", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}</small>
                </div>
                {post.authorId === account.id && (
                  <button className="post-delete" onClick={() => { setDeleteError(""); setDeleteTarget(post); }} aria-label="Excluir publicação" disabled={busy === `delete-${post.id}`}>
                    {busy === `delete-${post.id}` ? <LoaderCircle size={17} className="spin" /> : <Trash2 size={17} />}
                  </button>
                )}
              </header>
              {post.text && <p>{post.text}</p>}
              {post.image && <img className="post-image" src={post.image} alt="Imagem da publicação" />}
              ...
              <div className="comment-form">
                <input
                  ...
                  onChange={(event) => setComments({ ...comments, [post.id]: event.target.value })}
                  onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void addComment(post.id); } }}
                  placeholder="Escreva uma resposta"
                />
                <button onClick={() => void addComment(post.id)} aria-label="Enviar comentário" disabled={busy === `comment-${post.id}`}>
                  {busy === `comment-${post.id}` ? <LoaderCircle size={17} className="spin" /> : <Send size={17} />}
                </button>
              </div>
            </article>
          ))}
      ...
      <Modal
        open={composerOpen}
        onClose={() => { setComposerOpen(false); setMessage(""); setPublishConfirmOpen(false); setPublishError(""); }}
        eyebrow="COMUNIDADE"
        title="Compartilhar com a comunidade"
        dismissible={busy !== "publish"}
      >
        <form className="modal-form" onSubmit={requestPublish}>
          <Field label="O que você quer contar?">
            <textarea value={text} onChange={(event) => setText(event.target.value)} placeholder="Uma dúvida, atividade, descoberta ou conquista..." autoFocus maxLength={1200} />
          </Field>
          ...
          {imagePreview ? (
            <div className="image-preview"><img src={imagePreview} alt="Imagem escolhida" /><button type="button" onClick={clearImage}>Tirar imagem</button></div>
          ) : (
            <button className="upload-button" type="button" onClick={() => fileRef.current?.click()}><ImagePlus size={20} /> Escolher imagem</button>
          )}
          {message && <p className="form-error" role="alert">{message}</p>}
          <div className="modal-action-row">
            <button className="secondary-button" type="button" onClick={() => setComposerOpen(false)} disabled={busy === "publish"}>Cancelar</button>
            <LoadingButton className="primary-button" type="submit" disabled={!text.trim() && !imageFile} loading={false}>Publicar</LoadingButton>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmDialog
        ...
        text="Seu conteúdo ficará visível para as pessoas da comunidade. Deseja publicar agora?"
        confirmLabel="Confirmar publicação"
        busy={busy === "publish"}
        error={publishError}
        onCancel={() => { if (busy !== "publish") { setPublishConfirmOpen(false); setPublishError(""); } }}
      ...
      <ConfirmDialog
        ...
        text="A publicação e os comentários serão removidos da comunidade."
