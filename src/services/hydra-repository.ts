...
export async function loadCommunityFeed(): Promise<CommunityPost[]> {
  ...
  return ((data ?? []) as Row[]).map((post) => ({
    ...
    state: post.state ? String(post.state) : undefined,
...
export async function loadAccount(user: User): Promise<HydraAccount> {
  ...
  const goals = typeof profile.premium_goals === "object" && profile.premium_goals ? profile.premium_goals as Row : {};
  base.settings = {
  ...
  base.property = {
    ...
    municipalityIbgeCode: property?.municipality_ibge_code ? String(property.municipality_ibge_code) : undefined,
    state: String(property?.state ?? ""),
    stateName: property?.state_name ? String(property.state_name) : undefined,
    postalCode: String(property?.postal_code ?? ""),
    region: property?.region ? String(property.region) : undefined,
    street: property?.street ? String(property.street) : undefined,
    district: property?.district ? String(property.district) : undefined,
    addressComplement: property?.address_complement ? String(property.address_complement) : undefined,
    ddd: property?.ddd ? String(property.ddd) : undefined,
  ...
  base.waterSources = ((sources.data ?? []) as Row[]).map((row) => ({ id: String(row.id), name: String(row.name), type: String(row.source_type), status: row.status as "ativa" | "atenção" | "inativa" }));
  base.waterRecords = ((records.data ?? []) as Row[]).map((row) => ({ id: String(row.id), date: dateString(row.recorded_on), amount: Number(row.amount), sourceId: String(row.source_id), purpose: String(row.purpose), note: row.note ? String(row.note) : undefined }));
  base.animals = await Promise.all(((animals.data ?? []) as Row[]).map(async (row) => ({
    id: String(row.id), identification: String(row.identification), name: row.name ? String(row.name) : undefined, species: String(row.species), breed: row.breed ? String(row.breed) : undefined, sex: row.sex ? String(row.sex) : undefined, birthDate: row.birth_date ? dateString(row.birth_date) : undefined, weight: row.weight === null || row.weight === undefined ? undefined : Number(row.weight), photoPath: row.photo_path ? String(row.photo_path) : undefined, photoUrl: await signedPrivateUrl(row.photo_path as string | undefined), status: String(row.status), electronicId: row.electronic_id ? String(row.electronic_id) : undefined, notes: row.notes ? String(row.notes) : undefined, history: Array.isArray(row.history) ? row.history as never[] : [],
  })));
  base.sectors = ((sectors.data ?? []) as Row[]).map((row) => ({ id: String(row.id), name: String(row.name), kind: String(row.kind), note: row.note ? String(row.note) : undefined }));
  base.activities = ((activities.data ?? []) as Row[]).map((row) => ({ id: String(row.id), title: String(row.title), category: String(row.category), date: dateString(row.activity_date), sectorId: row.sector_id ? String(row.sector_id) : undefined, animalId: row.animal_id ? String(row.animal_id) : undefined, note: row.note ? String(row.note) : undefined, done: Boolean(row.done) }));
  base.monitoring = await Promise.all(((monitoring.data ?? []) as Row[]).map(async (row) => {
    ...
    return { id: String(row.id), date: dateString(row.monitored_on), sectorId: row.sector_id ? String(row.sector_id) : undefined, type: String(row.monitoring_type), duration: row.duration ? String(row.duration) : undefined, note: row.note ? String(row.note) : undefined, occurrence: row.occurrence ? String(row.occurrence) : undefined, photoPaths, photoUrls: (await Promise.all(photoPaths.map(signedPrivateUrl))).filter(Boolean) as string[] };
  }));
...
async function syncCollection<T extends { id: string }>(table: string, previous: T[], next: T[], mapRow: (item: T) => Row) {
  const client = requireSupabase();
...
export async function syncAccountDelta(previous: HydraAccount, next: HydraAccount) {
  ...
  if (changed(previous.profile, next.profile) || previous.phone !== next.phone || changed(previous.settings, next.settings)) {
    const { error } = await client.from("profiles").update({ full_name: next.profile.name, phone: next.phone, bio: next.profile.bio ?? null, water_alerts: next.settings.waterAlerts, push_notifications: next.settings.pushNotifications, premium_goals: next.settings.premiumGoals }).eq("id", actor);
    throwIfError(error);
  }
  ...
  if (changed(previous.property, next.property)) {
    ...
    const { error } = await client.from("properties").upsert({
      ...
      municipality_ibge_code: next.property.municipalityIbgeCode ?? null,
      state: next.property.state,
      state_name: next.property.stateName ?? null,
      postal_code: next.property.postalCode || null,
      region: next.property.region ?? null,
      street: next.property.street ?? null,
      district: next.property.district ?? null,
      address_complement: next.property.addressComplement ?? null,
      ddd: next.property.ddd ?? null,
  ...
  await syncCollection("nfc_tags", previousLinked.map((item) => ({ ...item, id: item.id.replace(/-electronic$/, "-tag") })), nextLinked.map((item) => ({ ...item, id: item.id.replace(/-electronic$/, "-tag") })), (item) => ({ owner_user_id: owner, id: item.id, animal_id: item.animalId, code: item.code, technology: "NFC/RFID" }));
}
...
export async function signInWithGoogle() {
  ...
  const redirectTo = native ? "br.com.hydraagro.app://auth/confirm" : window.location.origin;
  const { data, error } = await requireSupabase().auth.signInWithOAuth({ provider: "google", options: { redirectTo, skipBrowserRedirect: native } });
  throwIfError(error);
  if (native) {
...
export async function resetPassword(email: string) {
  const redirectTo = Capacitor.isNativePlatform() ? "br.com.hydraagro.app://auth/recovery" : new URL("/auth/recovery", window.location.origin).toString();
...
export async function togglePostLike(userId: string, post: CommunityPost) {
  const client = requireSupabase();
  const operation = post.liked ? client.from("likes").delete().eq("post_id", post.id).eq("user_id", userId) : client.from("likes").insert({ post_id: post.id, user_id: userId });
...
export async function loadPublicAdminContent() {
  ...
  return { announcements: ((announcements.data ?? []) as Row[]).map(mapAnnouncement), links: ((links.data ?? []) as Row[]).map(mapLink) };
}
...
export async function loadModerationPosts(): Promise<CommunityPost[]> {
  ...
  const { data: profileData, error: profileError } = authorIds.length ? await client.from("profiles").select("id,full_name,avatar_path").in("id", authorIds) : { data: [], error: null };
  ...
  return rows.map((row) => {
    ...
    return { id: String(row.id), authorId, author: String(profile?.full_name || "Produtor"), authorAvatarUrl: publicMediaUrl("avatars", profile?.avatar_path as string | undefined), text: String(row.body ?? ""), date: String(row.created_at), image: publicMediaUrl("community-media", row.image_path as string | undefined), likes: 0, liked: false, comments: [], moderationStatus: row.moderation_status as CommunityPost["moderationStatus"] };
  });
}
...
export async function setUserSubscription(userId: string, enablePlus: boolean, premiumUntil?: string) {
  const { error } = await requireSupabase().rpc("admin_set_subscription", { target_user_id: userId, enable_plus: enablePlus, premium_until: premiumUntil ? new Date(`${premiumUntil}T23:59:59-03:00`).toISOString() : null });
  throwIfError(error);
}
