-- Reserva a Fazenda Bananeira (Ellen) como 2º lugar no ranking.
...
as $$
  with identity as (
    select
      ...
      exists(select 1 from public.properties p where p.owner_user_id = p_owner and lower(trim(p.name)) = 'fazenda bananeira') as is_ellen_farm
  ), stats as (
    select
      identity.is_app_owner,
      identity.is_ellen_farm,
